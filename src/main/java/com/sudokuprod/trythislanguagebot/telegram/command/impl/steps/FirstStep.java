...
import static com.sudokuprod.trythislanguagebot.telegram.utils.ConstantText.FIRST_QUESTION;

@Slf4j
@Component
@RequiredArgsConstructor
public class FirstStep extends ParentByStepAndResult {
    ...
    public void execute(@NonNull final TelegramLongPollingBot bot,
        ...
        final String[][] buttons = new String[][]{{"Мой выбор - сказка!", "call1"},
                {"Конечно с массажем спины!", "call2"}, {"Есть третий стул?", "call3"},
                {"При чем здесь стулья вообще?", "call4"}};
        ...
        bot.execute(responseCreator.createPhoto(message.getChatId(), "static/images/1.jpg",
                profile.getName() + FIRST_QUESTION, keyboardBuilder.createInlineKeyboard(keyboard)));
